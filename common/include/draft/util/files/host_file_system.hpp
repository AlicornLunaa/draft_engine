...
namespace Draft {
    /**
     * @brief A filesystem that only ever talks to disk, via DiskFileProvider.
     *
     * Unlike AssetFileSystem, there's no provider search here, this is for code that
     * specifically needs to read/write loose files (project files, exported builds, editor
     * state, ...) rather than resolve a logical asset path that might be embedded.
     */
