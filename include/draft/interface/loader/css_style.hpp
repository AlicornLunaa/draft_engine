...
namespace Draft {
    namespace UI {
        struct Rule {
            ...
            CSSLoader::PseudoSelector pseudoSelector;
            std::map<std::string, std::any> properties; // Points Property -> value
        };

        class Stylesheet {
        private:
            std::array<std::unordered_map<std::string, Rule>, CSSLoader::PSEUDO_COUNT> styles; // Points selector -> rules, where each index is a different pseudo selector
        ...
        public:
            ...
            template<typename T>
            std::optional<T> get(Layout& layout, const std::string& property, CSSLoader::PseudoSelector pseudo = CSSLoader::NONE) const {
            ...
            template<typename T>
            T get(Layout& layout, const std::string& property, T defaultValue, CSSLoader::PseudoSelector pseudo = CSSLoader::PseudoSelector::NONE) const {
            ...
            template<typename T>
            void set(const std::vector<std::string>& selectors, CSSLoader::PseudoSelector pseudoSelector, const std::string& property, const T& value){
                // Sets this css rule
