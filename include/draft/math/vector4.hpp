...
namespace Draft {
    ...
    public:
        ...
        Vector4<T>& operator/= (T r){ x /= r; y /= r; z /= r; w /= r; return this; }
        Vector4<T> operator+ (const Vector4<T> &r){ return { x + r.x, y + r.y, z + r.z, w + r.w }; }
        Vector4<T> operator- (const Vector4<T> &r){ return { x - r.x, y - r.y, z - r.z, w - r.w }; }
        Vector4<T> operator* (T r){ return { x * r, y * r, z * r, w * r }; }
        Vector4<T> operator/ (T r){ return { x / r, y / r, z / r, w / r }; }
        T operator* (const Vector4<T> &r){ return (x * r.x + y * r.y + z * r.z + w * r.w); }
        bool operator== (const Vector4<T> &r){ return (x == r.x && y == r.y && z == r.z && w == r.w); }
        bool operator!= (const Vector4<T> &r){ return (x != r.x || y != r.y || z != r.z || w != r.w); }
