...
#define DRAFT_EXPOSE_SERIALIZER(type)                                                   \
    friend size_t Serializer::serialize(type& obj, std::vector<std::byte>& byteArray);  \
    friend size_t Serializer::deserialize(type& obj, std::byte* bytePtr);               \
    friend void Serializer::serialize(type& obj, nlohmann::json& json);                 \
    friend void Serializer::deserialize(type& obj, nlohmann::json& json);

// Main declarations here
namespace Draft {
    // Would make this just a namespace but I'd like to use friends for more encapsulation
    namespace Serializer {
        // Generic specialization, itll work for basic shallow data types. Anything complex, using the heap, etc, has to be defined in the source file
        template<typename T>
