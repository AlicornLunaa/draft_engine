...
namespace Draft {
    // Raw functions
    b2BodyType convert_to_b2(BodyType type){
        switch(type){
    ...
    BodyType convert_to_draft(b2BodyType type){
        switch(type){
    ...
    RigidBody::~RigidBody(){
        auto colliders = std::move(attachedColliders);
    ...
    Fixture* RigidBody::get_fixture(void* ptr) const {
        ...
        auto iter = this->ptr->b2ToFixturePtrs.find(ptr);
        return iter != this->ptr->b2ToFixturePtrs.end() ? iter->second : nullptr;
    }
    ...
    BodyType RigidBody::get_type() const { return convert_to_draft(ptr->body->GetType()); }
    ...
    bool RigidBody::is_sleep_allowed() const { return ptr->body->IsSleepingAllowed(); }
