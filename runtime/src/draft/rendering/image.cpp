...
namespace Draft {
    ...
    std::byte float_to_byte(float value){
        ...
        unsigned char intermediate = (clamped * 255);
    ...
    float byte_to_float(std::byte byte){
        ...
        float val = reinterpret_cast<unsigned char&>(byte);
        return val / 255.f;
    }
    ...
    void Image::copy(const Image& src, const Vector2i& position, IntRect rect, bool applyAlpha){
        ...
        for(int y = start.y; y < (int)end.y; y++){
            for(int x = start.x; x < (int)end.x; x++){
                ...
                if(x + position.x >= (int)size.x || x + position.x < 0 || y + position.y >= (int)size.y || y + position.y < 0)
                    continue;
                ...
                for(unsigned int i = 0; i < destStride; i++){
                    ...
                    if(i < srcStride){
                        ...
                        if(applyAlpha && src.format == RGBA && i != 3){
                            float scalar = byte_to_float(src.dataPtr[(x + y * src.size.x) * srcStride + 3]); // Alpha byte to float
