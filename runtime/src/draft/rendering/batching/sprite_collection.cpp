...
namespace Draft {
    namespace {
        // Lazily-constructed, process-lifetime default sprite shader. Deliberately leaked via `new`.
        Shader& default_shader(){
            static Shader* shader = new Shader(AssetFileSystem().open("assets/shaders/default"));
            return *shader;
        }
    }
    ...
    void SpriteCollection::draw(SpriteProps props){
        ...
        if(!props.material.shader){
            props.material.shader = &default_shader();
        }
