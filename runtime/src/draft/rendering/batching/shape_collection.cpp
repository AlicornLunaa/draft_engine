...
namespace Draft {
    // Static data
    Resource<Shader> ShapeCollection::default_shader(){
        // Lazily-constructed, process-lifetime shared default shader. Deliberately leaked via `new`.
        static std::shared_ptr<AssetSlot<Shader>>* slot = new std::shared_ptr<AssetSlot<Shader>>(
            std::make_shared<AssetSlot<Shader>>(
                std::make_shared<Shader>(AssetFileSystem().open("assets/shaders/shapes"))
            )
        );
    ...
    void ShapeCollection::draw_line(const Vector2f& start, const Vector2f& end){
        ...
        if(m_currentRenderType != ShapeRenderType::LINE){
            Logger::println(LogLevel::Warning, "ShapeCollection", "draw_line(const Vector2f&, const Vector2f&) may only be called with LINE render type.\n\tIt was set automatically, but you should do it manually.");
    ...
    void ShapeCollection::draw_arrow(const Vector2f& head, const Vector2f& tail, float arrowScale){
        // Lines can only be GL_LINES
        if(m_currentRenderType != ShapeRenderType::LINE){
            Logger::println(LogLevel::Warning, "ShapeCollection", "draw_arrow(const Vector2f&, const Vector2f&) may only be called with LINE render type.\n\tIt was set automatically, but you should do it manually.");
            set_render_type(ShapeRenderType::LINE);
        }
