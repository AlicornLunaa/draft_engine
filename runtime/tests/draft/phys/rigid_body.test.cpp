...
TEST(RigidBody, CreateAndDestroy)
{
    ...
    Draft::BodyDef def;
    def.type = Draft::BodyType::STATIC;
    def.position = {1.0f, 2.0f};
    def.angle = 0.5f;

    Draft::RigidBody* body = world.create_rigid_body(def);
    ASSERT_NE(body, nullptr);
    ASSERT_EQ(body->get_type(), Draft::BodyType::STATIC);
...
TEST(RigidBody, GetFixtureReturnsNullForUnknownHandle)
{
    Draft::World world({0.f, 0.f});
    Draft::BodyDef def;
    Draft::RigidBody* body = world.create_rigid_body(def);

    ASSERT_EQ(body->get_fixture(reinterpret_cast<void*>(0x1234)), nullptr);
