...
static void create_dummy_bodies(Draft::World& world, Draft::RigidBody*& bodyA, Draft::RigidBody*& bodyB) {
    Draft::BodyDef defA;
...
TEST(GearJointDef, DefaultConstructorHasNullConstituentJoints)
{
    Draft::World world({0.f, 0.f});
    Draft::RigidBody *bodyA, *bodyB;
    create_dummy_bodies(world, bodyA, bodyB);

    Draft::GearJointDef def(bodyA, bodyB);
    ASSERT_EQ(def.type, Draft::Joint::Type::GEAR);
    ASSERT_EQ(def.joint1, nullptr);
    ASSERT_EQ(def.joint2, nullptr);
    ASSERT_EQ(def.ratio, 1.f);

    world.destroy_body(bodyA);
    world.destroy_body(bodyB);
}

TEST(PulleyJointDef, DefaultCollideConnectedIsTrue)
{
    // Unlike every other *JointDef, PulleyJointDef's collide param defaults to true, not false.
    Draft::World world({0.f, 0.f});
    Draft::RigidBody *bodyA, *bodyB;
    create_dummy_bodies(world, bodyA, bodyB);

    Draft::PulleyJointDef def(bodyA, bodyB);
    ASSERT_EQ(def.type, Draft::Joint::Type::PULLEY);
    ASSERT_TRUE(def.collideConnected);
