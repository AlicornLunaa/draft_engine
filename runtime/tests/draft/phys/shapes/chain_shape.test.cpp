...
// box2d's own b2ChainShape::TestPoint is hardcoded to always return false
TEST(ChainShape, ContainsAlwaysFalse)
{
    Draft::ChainShape chain(Draft::ChainShape::ChainType::LOOP);
    chain.add({0.f, 0.f});
    chain.add({10.f, 0.f});
    ...
    ASSERT_FALSE(chain.contains({0.f, 0.f}));    // Vertex
    ASSERT_FALSE(chain.contains({5.f, 5.f}));    // Inside the loop
