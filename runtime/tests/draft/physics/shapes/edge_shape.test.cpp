...
// box2d's own b2EdgeShape::TestPoint is hardcoded to always return false (an infinitely-thin
// edge has no interior).
TEST(EdgeShape, ContainsAlwaysFalse)
{
    Draft::EdgeShape edge({0.f, 0.f}, {10.f, 0.f}); // Horizontal line segment

    ASSERT_FALSE(edge.contains({0.f, 0.f}));       // Exactly on the segment
    ASSERT_FALSE(edge.contains({5.f, 0.f}));        // Exactly on the segment
    ...
    ASSERT_FALSE(verticalEdge.contains({0.f, 5.f}));
