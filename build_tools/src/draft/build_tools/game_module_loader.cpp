...
namespace Draft {
    ...
    GameModuleLoader::GameModuleLoader(const std::filesystem::path& path){
        ...
        if(!m_registerGame){
            // Loaded fine but isn't a game module (no DRAFT_GAME_MODULE), don't leak the handle.
            #if defined(_WIN32)
            ...
            throw std::runtime_error("GameModuleLoader: '" + path.string() + "' doesn't export draft_register_game() (missing DRAFT_GAME_MODULE?)");
        }
    }
